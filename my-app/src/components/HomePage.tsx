  ...
  return (
    <div className="bg-slate-50 min-h-screen text-slate-800 font-sans overflow-hidden relative">
      ...
      <section className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 p-28 pb-20 flex flex-col items-center text-center">
        ...
        >
          <motion.div variants={fadeUp} className="inline-flex items-center px-4 py-2 rounded-full bg-blue-100/50 text-blue-700 font-medium text-sm mb-8 border border-blue-200 backdrop-blur-sm">
            <span className="flex h-2 w-2 rounded-full bg-blue-600 mr-2 animate-ping"></span>
            Nền tảng học tập trực tuyến hàng đầu
          </motion.div>
          
          <motion.h1 variants={fadeUp} className="text-5xl md:text-7xl font-extrabold tracking-tight leading-tight text-slate-900 mb-6">
          ...
          <motion.p variants={fadeUp} className="max-w-2xl text-lg md:text-xl text-slate-600 mb-10">
            Nâng tầm kỹ năng của bạn với hơn <span className="font-bold text-blue-600">213K</span> khóa học video trực tuyến chất lượng cao được cập nhật mỗi tháng.
          </motion.p>

          <motion.div variants={fadeUp} className="flex flex-col sm:flex-row gap-4 items-center justify-center w-full sm:w-auto">
            <Link href={PUBLIC_PATH.COURSES} className="w-full sm:w-auto px-8 py-4 bg-gradient-to-r from-blue-600 to-indigo-600 text-white font-bold rounded-2xl shadow-lg shadow-blue-500/30 hover:shadow-blue-500/50 hover:-translate-y-1 transition-all duration-300">
              Khám phá khóa học 🚀
            </Link>
      ...
      <section className="relative z-10 max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 mb-24">
        ...
        >
          ...
          <h2 className="text-2xl md:text-4xl font-bold text-white leading-snug mb-10 relative z-10">
            Phát triển kỹ năng với hơn <span className="text-transparent bg-clip-text bg-gradient-to-r from-blue-400 to-teal-300 text-5xl">200+</span> khóa học <br className="hidden md:block" /> từ các trường đại học & tổ chức danh tiếng.
          </h2>
      ...
      <section className="relative z-10 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
        ...
        >
          {[
            { 
              title: "Backend 54",
              maKhoaHoc: "123456736", // <--- Thêm mã khóa học để làm URL
              image: "https://elearningnew.cybersoft.edu.vn/hinhanh/backend-54.jpg" 
            },
            { 
              title: "Test FrontEnd",
              maKhoaHoc: "1234567890123",
              image: "https://elearningnew.cybersoft.edu.vn/hinhanh/test-frontend.png" 
            },
            { 
              title: "Fullstack Web Development",
              maKhoaHoc: "13485",
              image: "https://elearningnew.cybersoft.edu.vn/hinhanh/fullstack-web-development_gp01.jpg" 
            }
          ].map((course, idx) => (
            <motion.div variants={fadeUp} key={idx} className="bg-white rounded-3xl p-4 shadow-[0_8px_30px_rgb(0,0,0,0.04)] hover:shadow-[0_8px_30px_rgb(59,130,246,0.12)] hover:-translate-y-2 transition-all duration-300 border border-slate-100 group cursor-pointer">
              ...
              {/* Click vào hình ảnh cũng chuyển hướng luôn */}
              <Link href={`/courseDetail/${course.maKhoaHoc}`} className="block h-48 rounded-2xl mb-6 relative overflow-hidden bg-slate-100">
                <img 
                  src={course.image} 
                  alt={course.title} 
              ...
              <div className="px-2">
                ...
                <div className="flex items-center justify-between mt-6 pt-4 border-t border-slate-100">
                  ...
                  {/* Thay div thành thẻ Link trỏ tới trang chi tiết khóa học */}
                  <Link 
      ...
      <section className="py-24 relative z-10 bg-white/50 backdrop-blur-xl border-y border-slate-200">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 text-center">
          ...
          >
            {['Lập trình BackEnd', 'Lập trình FrontEnd', 'Lập trình Full Stack', 'Lập trình di động', 'Tư duy lập trình', 'Thiết kế web'].map((cat, i) => (
              <motion.div 
                variants={{ hidden: { opacity: 0, scale: 0.8 }, visible: { opacity: 1, scale: 1, transition: { type: "spring", stiffness: 100 } } }}
                key={i} className="bg-white border border-slate-200 rounded-2xl p-6 flex flex-col items-center shadow-sm hover:shadow-xl hover:border-blue-300 hover:-translate-y-1 transition-all duration-300 cursor-pointer group"
              >
                ...
                <h3 className="text-sm font-bold text-slate-700 text-center">{cat}</h3>
              </motion.div>
            ))}
          </motion.div>
        </div>
      ...
      <section className="py-24 relative z-10 max-w-5xl mx-auto px-4">
        ...
        >
          ...
          <div className="relative z-10">
            <h2 className="text-3xl md:text-5xl font-black text-white mb-6">Bạn đã sẵn sàng để bắt đầu?</h2>
            <p className="text-blue-100 text-lg mb-10 max-w-2xl mx-auto">
              Đăng ký ngay hôm nay để nhận thông báo về các khóa học mới nhất và những ưu đãi độc quyền dành riêng cho bạn.
            </p>
            <div className="flex flex-col sm:flex-row justify-center max-w-md mx-auto gap-3">
              <input type="email" placeholder="Nhập email của bạn..." className="flex-1 px-6 py-4 rounded-xl text-slate-900 font-medium focus:outline-none focus:ring-4 focus:ring-white/30 shadow-inner" />
              <motion.button 
