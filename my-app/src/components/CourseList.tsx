...
// import { useCoursePagination } from "../hook/useCoursePagination";
...
//   React.useEffect(() => {
//     handlePageChange(1);
//   }, [selectedCategory, keyword, handlePageChange]);

//   const isLoading = paginationLoading || listLoading;
//   const hasError = paginationError || listError;
...
const CourseList = () => {
  const {
    ...
    categoryLoading,
    categoryError,
    filteredCourses, // Lấy mảng gốc ra xài trực tiếp
  } = useCourseList();
  ...
  // Phân trang đơn giản, mượt mà và không bao giờ lỗi
  ...
  return (
    <section className="max-w-7xl mx-auto px-4 py-8 bg-[#f4f7f9]">
      <div className="grid grid-cols-1 md:grid-cols-12 gap-8">
        ...
        <aside className="md:col-span-3 space-y-6">
          <div className="bg-white p-6 shadow-sm border-t-4 border-[#00a2e8] rounded-lg">
            ...
            <div className="space-y-3">
              ...
              {categories.map((cat: Category) => {
                const isActive = selectedCategory === String(cat.maDanhMuc);
                return (
                  <button
                    key={cat.maDanhMuc}
                    type="button"
                    onClick={() => handleCategorySelect(String(cat.maDanhMuc))}
                    className={`w-full rounded-lg border px-4 py-3 text-left transition ${
                      isActive
                        ? "border-[#1a73e8] bg-blue-50 text-[#1a73e8]"
                        : "border-gray-200 text-gray-600 hover:border-blue-200 hover:text-blue-600"
        ...
        <main className="md:col-span-9">
          <div className="mb-6 border-b pb-4 border-gray-200">
            ...
            <p className="text-gray-500 text-sm mt-1">
              {keyword
                ? `Kết quả tìm kiếm "${keyword}": ${totalCourses} khóa học`
                : `${activeCategoryName}: ${totalCourses} khóa học`}
            </p>
          </div>
      ...
      {/* PHÂN TRANG */}
      {!categoryLoading && totalCourses > 0 && totalPages > 1 && (
